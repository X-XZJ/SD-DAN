...
from transforms.frequency_enhancement import HighLowFrequencyMixedEnhancement, AdaptiveFrequencyEnhancement
...
# 添加频域增强版本
hflip_freq = T.Compose([
    HighLowFrequencyMixedEnhancement(sigma=30, alpha=0.3, beta=0.3, gamma=0.4, apply_prob=0.5),
    T.RandomHorizontalFlip(p=0.5),
    T.PILToTensor(),
    T.ConvertImageDtype(torch.float),
    T.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)),
...
# 添加频域增强的strong_album版本 - 推荐用于病害检测
strong_album_freq = T.Compose([
    HighLowFrequencyMixedEnhancement(sigma=30, alpha=0.3, beta=0.3, gamma=0.4, apply_prob=0.7),
    T.RandomHorizontalFlip(),
    T.RandomChoice([
        T.RandomShortestSize(min_size=scales, max_size=1333, antialias=True),
        T.Compose([
...
# 添加频域增强的1200x2000版本
strong_album_1200_2000_freq = T.Compose([
    HighLowFrequencyMixedEnhancement(sigma=35, alpha=0.3, beta=0.3, gamma=0.4, apply_prob=0.7),
    T.RandomChoice([
        T.RandomShortestSize(min_size=scales_1200, max_size=2000, antialias=True),
        T.Compose([
            T.RandomShortestSize([600, 750, 900], antialias=True),
...
# 自适应频域增强版本 - 参数随机采样
strong_album_adaptive_freq = T.Compose([
    AdaptiveFrequencyEnhancement(
        sigma_range=(25, 40),
        alpha_range=(0.25, 0.35),
...
# 添加频域增强的RT-DETR版本
rtdetr_transform_freq = T.Compose([
    HighLowFrequencyMixedEnhancement(sigma=30, alpha=0.3, beta=0.3, gamma=0.4, apply_prob=0.6),
    T.RandomPhotometricDistort(p=0.8),
    T.RandomZoomOut(p=0.5, fill=0, side_range=(1.0, 4.0)),
    T.RandomIoUCrop(),
    T.RandomHorizontalFlip(p=0.5),
