...
def evaluate_acc(model, data_loader, epoch, accelerator=None):
    ...
    # 计算和打印精确率、召回率统计
    try:
        coco_evaluator.print_precision_recall_stats()
        # 获取详细结果
        detailed_results = coco_evaluator.calculate_precision_recall_metrics()
    except Exception as e:
        logger.warning(f"Failed to calculate detailed metrics: {e}")
        detailed_results = {}

    # print category-wise evaluation results (更新表格显示)
    cat_names = [cat["name"] for cat in coco.loadCats(coco.getCatIds())]
    table_data = [["class", "imgs", "gts", "dets", "recall@50", "precision@50", "ap@50"]]

    # table data for show
    bbox_coco_eval = coco_evaluator.coco_eval["bbox"]
    bbox_results = detailed_results.get("bbox", {})
    precision_per_class = bbox_results.get("precision_50", [])
    recall_per_class = bbox_results.get("recall_50", [])
    ...
    for cat_idx, cat_name in enumerate(cat_names):
        ...
        if len(cat_id) > 0:
            ...
            # 获取recall@50
            if cat_idx < len(recall_per_class):
                recall_val = recall_per_class[cat_idx]
            else:
                try:
                    recall_val = bbox_coco_eval.eval['recall'][0, cat_idx, 0, 2].item()
                except:
                    recall_val = 0.0
            
            # 获取precision@50
            if cat_idx < len(precision_per_class):
                precision_val = precision_per_class[cat_idx]
            else:
                precision_val = 0.0
            
            # 获取AP@50
            try:
                ap_val = bbox_coco_eval.eval['precision'][0, :, cat_idx, 0, 2].mean().item()
            except:
                ap_val = 0.0
            
            row_data += [f"{recall_val:.3f}", f"{precision_val:.3f}", f"{ap_val:.3f}"]
            table_data.append(row_data)

    # 计算平均值
    mean_recall = bbox_results.get("mean_recall_50", 0.0)
    mean_precision = bbox_results.get("mean_precision_50", 0.0)
    try:
    ...
    mean_data = ["mean results", "", "", "", f"{mean_recall:.3f}", f"{mean_precision:.3f}", f"{mean_ap50:.3f}"]
    ...
    eval_stats = {}
    if "model_time" in metric_logger.meters:
        eval_stats["eval_latency_ms_mean"] = metric_logger.meters["model_time"].global_avg * 1000.0
    
    # 添加我们计算的详细指标
    if "bbox" in detailed_results:
        bbox_metrics = detailed_results["bbox"]
        metric_dict.update({
            "Precision@50": bbox_metrics.get("mean_precision_50", 0.0),
            "Precision@75": bbox_metrics.get("mean_precision_75", 0.0),
            "Precision@50:95": bbox_metrics.get("mean_precision_avg", 0.0),
            "Recall@50": bbox_metrics.get("mean_recall_50", 0.0),
            "Recall@75": bbox_metrics.get("mean_recall_75", 0.0),
            "Recall@50:95": bbox_metrics.get("mean_recall_avg", 0.0),
        })

    if eval_stats.get("eval_latency_ms_mean") is not None:
        metric_dict["inference_latency_ms"] = eval_stats["eval_latency_ms_mean"]
        metric_dict["inference_fps"] = (
            1000.0 / eval_stats["eval_latency_ms_mean"] if eval_stats["eval_latency_ms_mean"] > 0 else 0.0
        )
    ...
    if "bbox" in detailed_results:
        bbox_metrics = detailed_results["bbox"]
        print(f"\n关键性能指标:")
        print(f"  mAP (0.5:0.95): {metric_dict['mAP']:.3f}")
        print(f"  AP@0.5       : {metric_dict['AP@50']:.3f}")
        print(f"  精确率@0.5   : {bbox_metrics.get('mean_precision_50', 0.0):.3f}")
        print(f"  召回率@0.5   : {bbox_metrics.get('mean_recall_50', 0.0):.3f}")
    print(f"{'='*80}")
    
    coco_evaluator.eval_stats = eval_stats
