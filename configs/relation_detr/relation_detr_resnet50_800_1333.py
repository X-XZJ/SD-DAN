...
# ===== 新增：导入并设置全局变量 =====
import models.bricks.relation_transformer as rt
rt.INIT_THRESHOLD = 0.5  # 设置 threshold 初始值
rt.FEATURE_ENHANCEMENT_CHANNELS = 256  # 设置特征增强通道数（与 embed_dim 一致）
# ====================================
...
# ===== 更新全局变量（确保与 embed_dim 一致）=====
rt.FEATURE_ENHANCEMENT_CHANNELS = embed_dim
# ==============================================
# 新增：threshold 和特征增强配置
init_threshold = 0.5  # threshold 初始值
sigma = 10.0          # 频率增强的 sigma 参数
temperature=1.0
...
# 新增：创建特征增强模块
# 注意：这里不直接创建，而是在 encoder 初始化后由 encoder 自己创建
# 因为 RelationTransformerEncoder 有 set_feature_enhancement 方法
...
transformer = RelationTransformer(
    encoder=RelationTransformerEncoder(
        ...
        # 注意：不在这里传入 feature_enhancement
        # 因为 encoder 会在 forward 时自动初始化
    ),
